...
##	################################################################
	...
	####################################################################
	####################################################################
	# open dns anyone?( writes config files to make pc use opendns )
	# NOTE: Not tested with wireless
	# NOTE: It is always better to setup the router to do this for all
	# boxes on your network.
	print 'Setting up Open DNS on the computer...'
	programFile = open('/etc/resolv.conf','w')
	temp = '# Generated by HackBox-Setup\n'
	temp += 'nameserver 208.67.222.222\n'
	temp += 'nameserver 208.67.220.220\n'
	programFile.write(temp)
	programFile.close()
	####################################################################
